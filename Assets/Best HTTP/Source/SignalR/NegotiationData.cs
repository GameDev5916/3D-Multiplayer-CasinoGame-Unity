    ...
    public sealed class NegotiationData
    {
            ...
            switch (req.State)
            {
                case HTTPRequestStates.Finished:
                    if (resp.IsSuccess)
                    {
                        ...
                        RaiseOnError(
                            $"Negotiation request finished Successfully, but the server sent an error. Status Code: {resp.StatusCode}-{resp.Message} Message: {resp.DataAsText} Uri: {req.CurrentUri}");
                    break;
        ...
        private static object Get(Dictionary<string, object> from, string key)
        {
            ...
            if (!from.TryGetValue(key, out value))
                throw new System.Exception($"Can't get {key} from Negotiation data!");
            return value;
        }
