...
namespace BestHTTP
{
    ...
    public sealed class SOCKSProxy : Proxy
    {
            ...
            try
            {
                ...
                if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                    HTTPManager.Logger.Information("SOCKSProxy",
                        $"Sending method negotiation - count: {count.ToString()} buffer: {BufferToHexStr(buffer, count)} ");
                ...
                if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                    HTTPManager.Logger.Information("SOCKSProxy",
                        $"Negotiation response - count: {count.ToString()} buffer: {BufferToHexStr(buffer, count)} ");
                    ...
                    switch (method)
                    {
                        ...
                        case SOCKSMethods.UsernameAndPassword:
                            if (this.Credentials.UserName.Length > 255)
                                throw new Exception(
                                    $"SOCKS Proxy - Credentials.UserName too long! {this.Credentials.UserName.Length.ToString()} > 255");
                            if (this.Credentials.Password.Length > 255)
                                throw new Exception(
                                    $"SOCKS Proxy - Credentials.Password too long! {this.Credentials.Password.Length.ToString()} > 255");
                            ...
                            if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                                HTTPManager.Logger.Information("SOCKSProxy",
                                    $"Sending username and password sub-negotiation - count: {count.ToString()} buffer: {BufferToHexStr(buffer, count)} ");
                            ...
                            if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                                HTTPManager.Logger.Information("SOCKSProxy",
                                    $"Username and password sub-negotiation response - count: {count.ToString()} buffer: {BufferToHexStr(buffer, count)} ");
                ...
                if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                    HTTPManager.Logger.Information("SOCKSProxy",
                        $"Sending connect request - count: {count.ToString()} buffer: {BufferToHexStr(buffer, count)} ");
                ...
                if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                    HTTPManager.Logger.Information("SOCKSProxy",
                        $"Connect response - count: {count.ToString()} buffer: {BufferToHexStr(buffer, count)} ");
                ...
                if (count < 10)
                    throw new Exception(
                        $"SOCKS proxy: not enough data returned by the server. Expected count is at least 10 bytes, server returned {count.ToString()} bytes! content: {BufferToHexStr(buffer, count)}");
        ...
        private void WriteString(byte[] buffer, ref int count, string str)
        {
            ...
            if (byteCount > 255)
                throw new Exception($"SOCKS Proxy - String is too large ({byteCount.ToString()}) to fit in 255 bytes!");
